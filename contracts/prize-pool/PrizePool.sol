...
abstract contract PrizePool is PrizePoolInterface, OwnableUpgradeable, ReentrancyGuardUpgradeable,TokenControllerInterface {
  ...
  uint256 public reserveUint;
  mapping(address => uint256) public reserveMap;
  ...
  AwardInfo[] public awardInfoArray;
    ...
    nonReentrant
  {
    ...
    _mint(to, amount, controlledToken, address(0),false);
    ...
    
  {
    ...
    _mint(to, amount, controlledToken, referrer,false);
    ...
    returns (uint256)
  {
    ...
    // burn the tickets
    beforeTokenTransferInternal(from,address(0),amount,controlledToken);
  ...
  /// @param to The address the tokens are being transferred to (0 if burning)
  /// @param amount The amount of tokens being trasferred
  function beforeTokenTransferInternal(address from, address to, uint256 amount,address controlledToken) internal  {
    if (from != address(0)) {
      uint256 fromBeforeBalance = IERC20Upgradeable(controlledToken).balanceOf(from);
      // first accrue credit for their old balance
      uint256 newCreditBalance = _calculateCreditBalance(from, controlledToken, fromBeforeBalance, 0);

      if (from != to) {
        // if they are sending funds to someone else, we need to limit their accrued credit to their new balance
        newCreditBalance = _applyCreditLimit(controlledToken, fromBeforeBalance.sub(amount), newCreditBalance);
      }

      _updateCreditBalance(from, controlledToken, newCreditBalance);
    }
    if (to != address(0) && to != from) {
      _accrueCredit(to, controlledToken, IERC20Upgradeable(controlledToken).balanceOf(to), 0);
    }
    // if we aren't minting
    if (from != address(0) && address(prizeStrategy) != address(0)) {
      prizeStrategy.beforeTokenTransfer(from, to, amount, controlledToken);
    }
  }
  ...
  function withdrawReserve(address to) external override virtual onlyReserve returns (uint256) {
    ...
    onlyControlledToken(controlledToken)
  {
    ...
    _mint(to, amount, controlledToken, address(0),true);
    ...
    awardInfoArray.push(AwardInfo(to,amount));
    emit Awarded(to, controlledToken, amount);
  }
  function getAwardInfoLength() external view returns (uint256) {
    return awardInfoArray.length;
  }
  function getLastAwardInfo(uint256 maxLength) external view returns (address[] memory,uint256[] memory) {
    uint256 length = awardInfoArray.length;
    if (awardInfoArray.length > maxLength){
      length =  maxLength;
    }
    address[] memory addrs = new address[](length);
    uint256[] memory  amounts = new uint256[](length);
    for (uint256 i =0;i<length;i++){
      AwardInfo memory awardInfo = awardInfoArray[awardInfoArray.length-i-1];
      addrs[i]=(awardInfo.user);
      amounts[i]=(awardInfo.amount);
    }
    return (addrs,amounts);
  }
  function getUserAwardInfo(address user) external view returns (uint256[] memory) {
    
    uint256[] memory amounts  = new uint256[](awardInfoArray.length);
    for (uint256 i =0;i<awardInfoArray.length;i++){
      AwardInfo memory awardInfo = awardInfoArray[i];
      if (user == awardInfo.user){
        amounts[i]=(awardInfo.amount);
      }
    }
    return (amounts);
  ...
  function _mint(address to, uint256 amount, address controlledToken, address referrer,bool cancelPrizeStrategy) internal {
    if (address(prizeStrategy) != address(0) && cancelPrizeStrategy== false) {
      prizeStrategy.beforeTokenMint(to, amount, controlledToken, referrer);
    }
    
    beforeTokenTransferInternal(address(0),to,amount,controlledToken);
  ...
  // function _updateCreditBalance2(address user, address controlledToken, uint256 newBalance) public {
  //   // uint256 oldBalance = _tokenCreditBalances[controlledToken][user].balance;

  //   // uint128 data1 = newBalance.toUint128();
  //   // uint32 data2 = _currentTime().toUint32();
  //   // CreditBalance memory c = CreditBalance( 123, 123, true);
  //   uint256 oldBalance = _tokenCreditBalances[controlledToken][user].balance;

  //   // require(false, "PrizePool/unknown-token");
  ...
  function setTokenParameters(address controlledToken,uint256 _reduceRate,uint256 _minimumChance,uint256 _notWinThreshold,uint256 _notWinOdds) external onlyOwner {
    ControlledToken(controlledToken).setParameters(_reduceRate, _minimumChance,_notWinThreshold,_notWinOdds);
