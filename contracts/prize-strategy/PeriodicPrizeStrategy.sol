...
abstract contract PeriodicPrizeStrategy is Initializable,
  ...
  function _awardTickets(address user, uint256 amount) internal {
    beforeTokenMintInternal(user, amount,  address(ticket), address(0));
  ...
  function beforeTokenMintInternal(
    address to,
    uint256 amount,
  ...
  function completeAward() external requireCanCompleteAward {
    bool isContract = isConflux();
    require(isContract,"cannot be invoked by contract!");
