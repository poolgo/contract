...
contract RNGBlockhash is RNGInterface, Ownable {
    ...
    function _isRequestComplete(uint32 requestId)
        internal
        view
        returns (bool)
    {
    ...
    //     uint256 hashValue = uint256(blockhash(block.number - 1))+block.timestamp;
    //     bytes32 data;
    //     if (hashValue % 2 == 0) {
    //         data = keccak256(abi.encodePacked(hashValue));
    //     } else {
    //         data = keccak256(
    //             abi.encodePacked(keccak256(abi.encodePacked(hashValue)))
    //         );
    //     }
    //     uint256 sum;
    ...
    //     }
    //     return
    //         uint8(data[sum % data.length]) *
    //         uint8(data[(sum + 2) % data.length]);
    // }
    function _getSeed() internal view virtual returns (uint256 seed) {
