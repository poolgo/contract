 ...
 */
 ...
 contract PoolControlPanel is StakePool,Ownable {
    ...
    function reBegin( uint256 _totalreward, uint256 _starttime, uint256 _stakingtime,uint256 _halvingPeriod) public onlyOwner {
         starttime = _starttime;
         stakingtime = _stakingtime;
         halvingPeriod = _halvingPeriod;
         notifyRewardAmount(_totalreward);
    }
     constructor(address _depositToken, address _rewardToken, uint256 _totalreward, uint256 _starttime, uint256 _stakingtime,uint256 _halvingPeriod) public {
     ...
     function stake(uint256 amount) public updateReward(msg.sender) checkOver checkStart{
         require(amount > 0, "ERROR: Cannot stake 0 Token");
         super._stake(amount);
         emit Staked(msg.sender, amount);
     }

     function withdraw(uint256 amount) public updateReward(msg.sender)  checkStart stakingTime{
    ...
    function exit() external stakingTime{
     ...
     function getReward() public updateReward(msg.sender)  checkStart stakingTime{
     ...
     function _getRewardInternal() internal updateReward(msg.sender)  checkStart{
         uint256 reward = earned(msg.sender);
         if (reward > 0) {
             rewards[msg.sender] = 0;
             rewardToken.safeTransfer(msg.sender, reward);
     ...
     modifier checkOver(){
         require(block.timestamp <= eraPeriod,"ERROR: already over");
         _;
     }
